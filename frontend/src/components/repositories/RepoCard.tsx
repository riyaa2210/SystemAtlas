...
import { GitBranch, Star, Globe, Lock, Trash2, Share2, BarChart2, MessageSquare, ExternalLink, Loader2, Zap, Layers } from "lucide-react";
          ...
          {isScanned && (
            <>
              <Link href={`/repositories/${repo.id}`}
                ...
                <Share2 className="w-3.5 h-3.5" />View Graph
              </Link>
              <Link href={`/repositories/${repo.id}?tab=architecture`}
                ...
                <Layers className="w-3.5 h-3.5" />View Architecture
              </Link>
              ...
              <Link href={`/copilot/${repo.id}`}
                className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg hover:bg-secondary/80 hover:text-amber-400 transition-colors text-muted-foreground">
