...
from app.core.architecture.architecture_inferrer import ArchitectureInferrer
    ...
    async def run_scan_pipeline(self, job_id: uuid.UUID, repo_id: uuid.UUID) -> None:
        ...
        async with AsyncSessionFactory() as db:
            ...
            try:
                ...
                # 5b. Infer high-level system architecture (stored separately from dep graph)
                await repo_repo.update_scan_job(job, stage="inferring_architecture")
                await db.commit()

                try:
                    arch_result = ArchitectureInferrer().infer(
                        results=results,
                        repo_languages=merged_languages,
                        repo_frameworks=manifest_frameworks,
                        manifest_deps=manifest_deps,
                    )
                    architecture_data = arch_result.to_dict()
                except Exception as arch_err:
                    logger.warning("Architecture inference failed (non-fatal)", error=str(arch_err)[:200])
                ...
                await repo_repo.create_snapshot(
                    ...
                    # Persist dependency graph in Postgres
                    graph_data={"nodes": graph_nodes, "edges": graph_edges},
                    # Persist system architecture diagram in Postgres
